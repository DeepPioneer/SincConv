...
class Adaptive_Spectral_Block(nn.Module):
    def __init__(self, L):
        ...
        self.complex_weight_high = nn.Parameter(torch.randn(L // 2 + 1, 2, dtype=torch.float32) * 0.02)  # 频域中的大小为 10//2 + 1
...
class model_classification(nn.Module):
    def __init__(self,dim=64,L=16000,num_classes=5):
        super().__init__()
...
if __name__ == "__main__":
    ...
    model = model_classification(dim=64, L=inputs.shape[2])
    output = model(inputs)
